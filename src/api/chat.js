...
export const getCustomerChatMessages = (customerId) => {
  const options = generateOptions(`chat/history/customer/${customerId}`);
  return (dispatch) => {
...
export const getUserChatMessages = (userId) => {
  const options = generateOptions(`chat/history/user/${userId}`, 'GET', { limit: 100, page: 1 });
  return (dispatch) => {
    dispatch(gettingChatMessages());
    return makeTheApiCall(options)
      .then(({ data }) => {
...
export const getGroupChatMessages = (groupId) => {
  const options = generateOptions(`chat/history/group/${groupId}`, 'GET', { limit: 100, page: 1 });
  return (dispatch) => {
    dispatch(gettingChatMessages());
    return makeTheApiCall(options)
      .then(({ data }) => {
