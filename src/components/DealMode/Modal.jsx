...
const DealModal = ({
  ...
  const [selectedSoldProducts, setSelectedSoldProducts] = useState([]);
  const [selectedPayments, setSelectedPayments] = useState([]);
  const [selectedDealElements, setSelectedDealElements] = useState([]);
  ...
  const [dealName, setDealName] = useState(
    getFullName(customerOverview?.customer)
  );
  ...
  const isSelectedDealElement = (deal) => selectedDealElements
    .find((dl) => dl.id === deal.id);

  const addProduct = (product) => {
    setSelectedProducts([...selectedProducts, {
      ...product,
      deliveryOn: moment().format('YYYY-MM-DD')
    }]);
  };
  ...
  const onDeliveryDateChange = (index, product) => {
    setSelectedProducts([
      ...
      product,
      ...selectedProducts.slice(index + 1, selectedSoldProducts.length)
    ]);
  };
        ...
        <div className="flex h-full">
          ...
          <div className="deal-mode__right-section">
            ...
            <TabPanel index={4} value={activeMenu}>
              <div className="products-list deal-mode__delivery" style={{ height: '420px' }}>
                {selectedProducts.slice(0, 2).map((product, index) => (
                  <div key={product.id} className={`flex items-center w-100 deal-mode-product-list-item-container ${index === 2 && 'selected'}`}>
                    <div className="products-list__item flex-1">
                      <div className="product-list-img">
                        <img alt="" src={getProductImgUrl(product)} />
                      </div>
                      <div className="flex-1">
                        ...
                        <div style={{ marginTop: '25px' }}>
                          <Input
                            ...
                            defaultValue={product.deliveryOn}
                            onChange={
                              (e) => onDeliveryDateChange(index, {
                                ...product,
                                deliveryOn: e.target.value
                              })
                            }
                          />
                        </div>
