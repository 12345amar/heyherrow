...
function AssignToDropDown(props) {
  ...
  */
  const fetchUsers = (filter = '') => {
    setloading(true);
    dispatch(getUsers(filter && `&search=${filter}`)).then(resp => {
      if (resp.length < 30) {
        setHasMore(false);
      }
      setloading(false);
  ...
  return (
    <div>
      { readOnly ?
      <UncontrolledDropdown id={id} className="tableOptions dropdown_wrapper dropdown_wrapper_assign">
         <DropdownToggle>
        ...
        : (
          <UncontrolledDropdown id={id} className="tableOptions dropdown_wrapper dropdown_wrapper_assign">
            ...
            <DropdownMenu>
              ...
              >
                {
                props.users.length > 0 ? props.users.map((data, i) => (
                  <DropdownItem key={i} onClick={() => { onSelect({ target: { value: data.id } }); setSelected(`${data.firstName} ${data.lastName}`); }} style={{ padding: 0 }}>
                    <div className="assignWrapper">
                      ...
                      <div style={{ width: '100%', marginLeft: '5px' }}>
                        ...
                        <div style={{ textAlign: 'justify' }}><span className="position">{get(data, 'role.name')}</span></div>
                      </div>
                    </div>
                  </DropdownItem>
                )) : <DropdownItem style={{ textAlign: 'center' }}>No record</DropdownItem>
              }
              </InfiniteScroll>
            </DropdownMenu>
          </UncontrolledDropdown>
