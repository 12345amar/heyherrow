...
import { getProductDetail, updateAdminProduct, createProductAsset, deleteAdminProductAsset, reorderCoverPhoto } from '../../../api/adminProducts';
import { onChangeAdminProduct } from '../../../redux/actions';
...
const ProductDetail = (props) => {
  ...
  const onDragEnd = (result) => {
    ...
    dispatch(reorderCoverPhoto(draggableId, {
  ...
  const onChange = (e, key, product, index) => {
    let value = ['modelYear', 'productStatusId'].includes(key) ? e.value : e.target.value;
    if (key === 'price') {
                ...
                client.domain == 'cbops' ? <>
                  ...
                  <div className="prodInput">
                    <TextField
                      ...
                      className="prodHalfSize"
                      disabled="true"
                      value={adminProductDetail.wholesalePrice || '-'}
                    />
                    <TextField
                      size="small"
                      label="Sales Date"
                    ...
                    {years.length > 0 && filtered.length >= 0 ? (
                      filtered.map((data, i) => (
                        <DropdownItem
                          key={i}
                          onClick={() => {
                            onChange(data, 'modelYear', adminProductDetail);
                            onSave(adminProductDetail, adminProductDetail.id, {
                    ...
                    {productStatuses && productStatuses.length > 0 ? (
                      productStatuses.map((data, i) => (
                        <DropdownItem
                          key={i}
                          onClick={() => {
                            onChange(data, 'productStatusId', adminProductDetail);
                            onSave(adminProductDetail, adminProductDetail.id, {
